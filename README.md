...
| 알람 | 근무 유형별 기상 알람. 켜면 기본 시각이 들어가고 그 근무인 날에만 울림 |
| 점검 | 매일/평일/매주/매월 반복 점검 항목 자동 생성, 체크 및 특이사항 메모, 진행률 |
| 설정 | 초기 설정 다시 하기, 교대 패턴 편집, 점검 항목 관리, 위젯 추가, 아침 근무 알림, 알림 권한 |

### 근무 기상 알람

**알람 탭**에서 근무 유형별로 켜기만 하면 됩니다.
켜는 순간 기본 시각(**주간 07:30 · 야간 16:00**, 그 외는 근무 시작 1시간 전)이 들어가고,
근무표에서 그 근무인 날에만 울립니다. **비번·휴무는 알람 대상이 아니며 목록에도 나오지 않습니다.**

- 시각·알람음·진동·다시 울림(안 함/5/10/15분)을 근무별로 따로 지정
- 알람음은 시스템 벨소리 선택기로 고름
- 잠금화면 위 전체화면 알람, 화면 자동 켜짐, 알람 볼륨으로 재생
- `setAlarmClock()` 으로 등록해 절전 모드에서도 정확히 울리고 재부팅 후 복구
- 끄면 설정이 삭제되어, 다시 켜면 항상 기본값에서 시작합니다
...
2. **근무 주기** — **교대 근무 시작일**(기본값은 오늘)을 정하고, 주기 구성을 편집한 뒤 **그 시작일이 몇 일차인지** 지정
3. **근무 시간** — 주기에 쓰인 근무의 시작·종료 시각 조정
4. **점검 루틴** — 자동 생성할 정기 점검 항목 선택 + 기존 수동 변경 정리 방식 선택

근무표는 **오늘이 아니라 교대 근무 시작일을 축으로** 계산됩니다.
시작일이 곧 사이클의 기준점이므로, 시작일을 바꾸면 이후 근무표 전체가 그에 맞춰 움직입니다.
...
**교대 근무 시작일** — 수습 기간처럼 교대 근무를 하지 않은 구간이 있으면 시작일을 지정할 수 있습니다.
그 이전 날짜는 **비워지지 않고 기본 주간 일정(평일 주간, 주말 휴무)으로 채워집니다.**
그 구간에 직접 지정해 둔 근무(연차·대타 등)가 있으면 그대로 유지됩니다.
초기 설정 2단계 또는 설정 › 교대 패턴에서 지정합니다.
...
변경한 날은 달력에 빨간 점으로 표시되고, 상세 카드에서 **원래 근무가 무엇이었는지** 확인할 수 있습니다.
야간으로 바꿀 때는 **다음 날도 비번으로 함께 변경**할 수 있습니다(기본 켜짐). 야간 근무 다음 날은 보통 비번이기 때문입니다.

**수동 변경 정리** — 직접 바꾼 근무는 패턴을 바꿔도 유지되므로, 스케줄을 다시 짜면 옛 변경이 남아 근무표가 어긋나 보일 수 있습니다.
정리하는 방법이 두 가지 있습니다.
- **설정 › 근무 설정 다시 하기** 마지막 단계에서 `시작일 이후만 지우기` / `전부 지우기` / `그대로 두기` 선택
  (기준이 오늘이 아니라 **교대 근무 시작일**이므로, 시작일 이전에 직접 바꿔 둔 근무는 보존됩니다)
- **설정 › 교대 패턴 세부 조정**의 `수동 변경 초기화` 버튼 (같은 선택지 제공)
...
- 잠금화면 위젯(`keyguard`) 카테고리로도 등록되어 있습니다. 삼성 Good Lock의 LockStar 등
  서드파티 위젯을 지원하는 잠금화면에서 목록에 나타날 수 있습니다(호스트 앱 지원 여부에 따라 다름).
...
| 서버로 올라감 | 기기에만 남음 |
| --- | --- |
| 근무 주기(패턴), 근무 유형과 시각, 날짜별 근무 변경(연차·대타) | 일정·할 일, 정기 점검 기록과 메모, 알람 설정 |
